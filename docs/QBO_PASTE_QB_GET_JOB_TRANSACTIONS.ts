// =============================================================================
// PASTE THIS ENTIRE FILE into Supabase Dashboard → Edge Functions → qb-get-job-transactions
// (Use when you can't deploy from the terminal; this file inlines _shared/qb.ts)
// =============================================================================

import { serve } from 'https://deno.land/std@0.168.0/http/server.ts'
import { createClient } from 'https://esm.sh/@supabase/supabase-js@2'

// ----- Inlined from _shared/qb.ts (no import so Dashboard works) -----
...
serve(async (req) => {
  if (req.method === 'OPTIONS') {
    return new Response('ok', {
      headers: { 'Access-Control-Allow-Origin': '*', 'Access-Control-Allow-Methods': 'POST, GET', 'Access-Control-Allow-Headers': 'authorization, x-client-info, apikey, content-type' }
    })
  }
  ...
  try {
    ...
    if (!authHeader) {
      return new Response(JSON.stringify({ error: 'No authorization header', transactions: [] }), { status: 401, headers: { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' } })
    }
    ...
    if (userError || !user) {
      return new Response(JSON.stringify({ error: 'Unauthorized', transactions: [] }), { status: 401, headers: { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' } })
    }

    const tokenResult = await getValidQbToken(supabaseClient, user.id)
    if (!tokenResult) {
      return new Response(JSON.stringify({ transactions: [], error: 'QuickBooks not connected' }), { status: 200, headers: { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' } })
    }

    const { accessToken, realmId } = tokenResult
    const apiBase = getQBApiBase()

    const accountRes = await qbFetch(apiBase, accessToken, realmId, `SELECT * FROM Account WHERE Active = true MAXRESULTS 1000`)
    ...
    pushFrom('Purchase', purchases, (t) => t.EntityRef?.name ?? t.EntityRef?.value ?? 'Unknown', (t) => {
    ...
    for (const r of [...(materialRows || []), ...(subRows || [])]) {
      if (r.qb_transaction_id && r.qb_transaction_type) importedSet.add(`${r.qb_transaction_type}:${r.qb_transaction_id}`)
    }
    const pending = outFiltered.filter((t) => !importedSet.has(`${t.qbTransactionType}:${t.qbTransactionId}`))
    ...
    return new Response(JSON.stringify({ transactions: [], error: (error as Error).message }), { status: 200, headers: { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' } })
  }
})
