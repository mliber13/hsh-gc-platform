...
| ~~P0-8~~ | ✅ **DONE 2026-07-31** — ~~apply pending migrations~~ | Verified vs remote SCHEMA (not just history): share-batch objects all present; `project_documents.file_path` was genuinely MISSING (repair had marked `025` applied without running DDL) → idempotent ADD COLUMN applied. App workaround removed: insert always stores `file_path`, fetch/delete select it, activating the previously-dead signed-URL regeneration. | `supabaseService.ts` (uploadProjectDocument / fetchProjectDocuments / deleteProjectDocument) |
...
- 🗣️ **Drywall billing — invoice from estimate via QuickBooks, sent from the app** _(added 2026-08-03; the opening move of a drywall billing module)._ Goal: the biller creates a QuickBooks invoice from a drywall project's estimate and sends it **without leaving the app** (no jumping into QB). **Reuses:** QB OAuth + the write pattern already proven by `qb-create-check` (→ new `qb-create-invoice` edge function); `qb-get-invoices` already reads invoices back for status. **Design forks to settle first (don't rush):** (1) **Billing model** — a single lump-sum invoice from the estimate total, or **progress/draw billing** (% complete per invoice)? Drywall is usually draw-based, and `ProjectedBillingsSection` already models a billing forecast — likely want draws, not lump sum. (2) **Customer mapping** — find-or-create a QB Customer from the project's client (mirror `qb-find-vendor`/vendor create for the customer side). (3) **Line items / item+account mapping** — map the drywall quote lines (or a summarized draw amount) to QB Items/income accounts. (4) **Send mechanism** — QB API `SendInvoice` (QB emails it) vs. app-side email. (5) **Idempotency/tracking** — store the returned QB invoice id on the project so we don't double-invoice and can sync status via `qb-get-invoices`. (6) **Permissions** — a billing capability/role gate. Scope: needs a design pass before build (schema for stored invoice refs + the create/send flow + a billing surface in the drywall project). Not v1.5.
- 🗣️ **Per-project job documents — hybrid app + Google Drive** _(decided 2026-08-03; hybrid, not all-or-nothing)._ Give each drywall project a document home for the prints, the customer scope-of-work doc, the PDF quote, and misc job docs — **without** reigniting storage/egress cost (see the 252 GB egress history; serving big print sets to mobile crew is exactly that risk). **Split by weight/origin:** (a) **In-app** (`project_documents` — infra already exists + hardened in P0-8): the light, app-generated, frequently-referenced docs — PDF quote, customer scope doc, small job docs; store these in the private Supabase bucket alongside field photos. (b) **Google Drive** for the **heavy print/plan sets** (50–200 MB) — don't upload into Supabase; instead store a **Drive folder/file link** on the project so the app stays the hub (one entry point, one tap to the prints for crew) while Google serves the bytes for free. **Build scope:** add a project-level Drive-link field (drywall project metadata) + surface the existing `project_documents` in the drywall project UI and in `/crew` (with the Drive link one tap away). **Open input:** how often crew open full prints on their phones — if constant, definitely keep prints in Drive and just link. Not v1.5.
