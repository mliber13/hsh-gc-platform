...
18. **Page-level tab strip → sidebar nav.** When migrating an in-page tab strip (Assumptions / Cash Flow / etc.) into sidebar items, drive selection via URL search param (`?tab=<x>`) and add a `selectCenterTab(tab)` helper that updates state AND URL together. Replace bare `setCenterTab(...)` callsites with the helper so programmatic tab changes (clear/reset flows, internal cross-links like a memo's "edit assumptions" link) don't leave the sidebar showing a stale active item. NavLink's default `isActive` ignores query strings — add a custom `matchTab` field on the sidebar item type and check it in `NavLinkItem` against `useLocation().search`. For any "active only when no query param" item (e.g., a top-level Dashboard that should deactivate once a tab is chosen), add a separate `matchExactNoQuery` flag. Reference: `DealWorkspace` ↔ `AppSidebar` deals nav.
19. **Cross-component modal triggering via query param.** When a sidebar action (or any other out-of-page control) needs to open a modal that lives inside a routed page component, don't extract the modal or use global events/context. Have the trigger navigate to `?<param>=1`, have the page useEffect-watch the param and open the modal + strip the param via `setSearchParams(..., { replace: true })` so refresh doesn't reopen. Single source of truth, no coupling, deep-linkable. Reference: `+ New Deal` in `AppSidebar` PrimaryAction → `/deals?new=1` → `DealWorkspace` opens its existing create modal.
20. **Light-mode color sweep on pages built dark-first.** Some early-built pages (`DealWorkspace` was the worst offender — ~30 spots) used dark-only utilities like `text-cyan-200`, `bg-emerald-950/40`, `text-blue-300`, `border-amber-700/60` that disappeared on the white background once light mode became the migration default. Triage with a single grep: `grep -nE '(text\|bg\|border)-(cyan\|emerald\|sky\|amber\|violet\|rose\|blue\|green\|red)-(100\|200\|300\|400\|800\|900\|950)' src/components/THE_FILE.tsx`. For each hit, choose between two recipes: (a) light/dark pair (`text-emerald-700 dark:text-emerald-300`) when you want different intensities per mode; (b) universal `bg-X-500/N + text-X-700 dark:text-X-300 + border-X-500/30` (status pill recipe §7) when one mid-tone reads cleanly in both modes. Prefer (b) for surfaces and pills, (a) for body text where dark mode looks better with a lighter shade. Map any `text-blue-X` to `text-sky-X` per playbook §6 — `blue` is non-standard.
...
- [ ] No dark-mode-only color literals remain unpaired with a light fallback (run `grep -nE '(text\|bg\|border)-(cyan\|emerald\|sky\|amber\|violet\|rose\|blue\|green\|red)-(100\|200\|300\|400\|800\|900\|950)' src/components/THE_FILE.tsx` — every hit should be inside a `dark:` modifier OR alongside a light-mode pair). See pitfall §20.
...
| Deal Workspace | ✅ Ported (partial) | `src/components/DealWorkspace.tsx` | 4839-line page. Outer wrapper is shell-friendly `flex h-[calc(100vh-3rem)]`; full slate-X palette swept to tokens; STAGE_COLOR uses the v0 pill recipe (amber/sky/emerald). Inline left deal-list aside dropped — deal switching now flows from `DealSelector` in AppHeader (mirror of `ProjectSelector`, gated on the deals workspace, lazy-loaded via `fetchDeals()`). Project Overview / Cost Summary were already side-by-side as a 6-col grid in the Assumptions tab; Deal Assistant kept as the existing `w-[340px]` right sidebar — v0's two-column layout was already in place once the left aside came out. In-page tab strip removed; tabs now driven by sidebar nav via `?tab=<x>` with a `selectCenterTab(tab)` helper that updates state + URL together (see pitfall §18). IA restructure: top-level **Dashboard** above the **Deal Analysis** group; **Overview** in Deal Analysis maps to the in-page Dashboard tab (Projected Profit / MOIC / Snapshot / ProForma Memo); Analysis & Insights + Documents folded into Deal Analysis (Documents disabled "Coming soon" — no in-page tab yet). **+ New Deal** moved from inline aside to sidebar `PrimaryAction`, signaled via `/deals?new=1` → DealsDashboard opens the shared `CreateDealDialog` (see pitfall §19). ProForma config dialog chrome tokenized. Light-mode color sweep: ~30 dark-only utilities (`text-cyan-200`, `bg-emerald-950/40`, `text-blue-300`, etc.) converted to light/dark pairs or the universal `bg-X-500/N` recipe (§7); zero `cyan` references remain. Capital Stack + Incentive Stack Remove buttons switched to 28px icon-only X (matching the phase-row pattern) — text "Remove" was overflowing `md:col-span-1` when the sidebar was expanded. The create-deal modal was extracted to `CreateDealDialog.tsx` so DealWorkspace and DealsDashboard can both mount it without coupling. **Deferred:** Documents nav item (no in-page tab yet). |
| Deals Dashboard | ✅ Ported | `src/components/DealsDashboard.tsx` | Mode A — list view at `/deals`. Mirror of `ProjectsDashboard`: summary cards (Total Deals / Active Pipeline / Total Projected Cost / Converted to Projects), search by name or location, status filter, sort by newest/oldest/name/cost/expected-start, deal rows with name + status pill + (Converted) badge + metadata + projected cost. Empty state with "+ Create Deal" CTA opening `CreateDealDialog`. `?new=1` handler picks up the sidebar "+ New Deal" navigation. Status pills use the §7 semantic recipe: emerald (active-pipeline), amber (pending-docs), violet (early-stage), sky (concept-pre-funding + very-early), muted (custom). New `/deals/workspace` route added so DealWorkspace tab nav has a stable home that doesn't depend on `/deals` (now the dashboard). |
