...
**Batch 3A SHIPPED 2026-09-23** (`2fdfbd1` + `ba08676`; briefs `docs/briefs/BATCH_3A_BLOB_LOST_UPDATE.md` and `..._PART2_PAGE_HELD_TIMESTAMP.md`). **Closes P0-DATA-1.** Took two passes, and the first one is worth recording because it looked finished: it guarded on an `updated_at` read *inside* the service a few milliseconds before the write, which compares current against current and always matches. Verified live — with a held timestamp and an out-of-band row bump, the part-1 comparison still succeeded while the page-held one returned zero rows. The plan's own phrase "spreads a **client-held** `prevLegacy`" was the tell, and the first brief did not chase it. Part 1 does close the supplier direction for real: `supplier_share_set_order_status` `jsonb_set`s only `{legacy,orders}` instead of read-replacing the whole column. Part 2 threads `updatedAtRaw` from the page into every write, with each write returning its new value so the nine sequential-write paths do not false-conflict on their second call. Accepted deliberately: `update_projects_updated_at` fires on any write to the row, so a status-pill change makes an open stage refuse its next save.

**Batch 3B SHIPPED 2026-09-23** (`11867eb` + `eca2cde`; brief `docs/briefs/BATCH_3B_PAYROLL_INTEGRITY.md`). **Closes P0-DATA-2, P0-DATA-3, P1-HR-1, P1-HR-3, P1-HR-4**; adds T9 and T12. Corrections to this plan found while grounding it: there are **five** writers of `pay_periods`, not three (`drywallLaborEntryEditService` has two of its own), and **`locked` lives inside the JSONB payload**, not as a column, so nothing server-side could see it. `save_pay_period` and `delete_pay_period` now hold org / stored-`locked` / page-held-`updated_at` guards, and one `select` is the only direct client access left. The delete path was **not** in the original brief — it is not a "writer" — while being the most destructive thing in the file: it could delete a locked run behind a client-only hidden button, and ran its reverse banked delta outside any transaction. **P1-HR-1's clamp turned out to be fixable rather than merely documentable:** `Math.max(0, …)` made the delta non-invertible, so a balance of 2 consumed by a run paying out 10 clamped to 0 and a naive reversal restored 10 — eight hours from nowhere. A per-person `baseline` now restores 2, and T12 round-trips the clamped case. Banked overdraw is **rejected, not clamped** (a clamp is one more silent drop); all 34 live pay periods were swept first and none trips the new rule. P0-DATA-3 was proved end to end with a throwaway off-roster punch, not just by its red test, because the fix is a refusal and a unit test cannot show what the dialog does with the throw.

**Batch 3 re-grounded 2026-09-23 before starting.** Two of its fourteen items were already closed: **P0-DATA-4** by the schedule-unification work (`persistCascadedDates` writes only `result.changes` and throws on the first error), and **P1-MONEY-8** as won't-fix per §8 Q5. Remaining work split as **3B** (P0-DATA-2/3, P1-HR-1/3/4, tests T9 + T12) and **3C** (P0-DATA-5, P1-DATE-1/2/3, P1-MONEY-9, tests T8 + T10).
