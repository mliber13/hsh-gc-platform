...
function sortActionItems(items: ActionItem[]): ActionItem[] {
  ...
  const rank = (item: ActionItem): number => {
    ...
    if (item.due_date < today) return 0
    return 1
  }
...
function renderActionItemsHtml(displayName: string, items: ActionItem[], actionItemsUrl: string): string {
  const today = localDateString(new Date())
  const listItems = items
    .map((item) => {
      const due = item.due_date ?? 'no due date'
      const overdue = item.due_date ? item.due_date < today : false
      return `<li style="margin: 0 0 8px;">
        <strong>${sanitize(item.task)}</strong> - Due: ${sanitize(due)} [${sanitize(item.status)}]
        ${overdue ? '<em style="color: #c2410c;"> (overdue)</em>' : ''}
      </li>`
    })
    .join('')
...
<html>
  <body style="font-family: Arial, sans-serif; line-height: 1.5; color: #1f2937; margin: 0; padding: 24px; background: #f8fafc;">
    <div style="max-width: 600px; margin: 0 auto; background: #ffffff; border: 1px solid #e5e7eb; border-radius: 10px; padding: 24px;">
      ...
      <p style="margin: 0 0 12px;">Here's where your action items stand going into this week:</p>
      <ul style="padding-left: 20px; margin: 0 0 16px;">
        ${listItems}
      </ul>
      <p style="margin: 0 0 20px;">
        <a href="${actionItemsUrl}" style="display: inline-block; padding: 10px 14px; background: #0e79c9; color: #ffffff; text-decoration: none; border-radius: 6px; font-weight: 600;">
  ...
  try {
    ...
    for (const lead of activeLeads) {
      try {
        ...
        const items = sortActionItems((rawItems ?? []) as ActionItem[])
        if (items.length === 0) {
          summary.skipped_no_items += 1
          continue
        }

        const html = renderActionItemsHtml(
          lead.display_name,
          items,
          `${appBaseUrl.replace(/\/$/, '')}/action-items`,
        )
        ...
        const messageId = await sendResendEmail({
          ...
          from: fromEmail,
          to: email,
          subject: `Your open action items - ${items.length} open`,
          html,
        })
        ...
        console.error('send-meeting-action-items-digest lead error', {
          leadId: lead.id,
    ...
    const message = error instanceof Error ? error.message : 'Unknown error'
    console.error('send-meeting-action-items-digest fatal error', { error: message })
