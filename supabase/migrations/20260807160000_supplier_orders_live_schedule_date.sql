-- The operator supplier board showed each order's STORED deliveryDate from metadata,
-- so it didn't follow schedule changes — unlike the supplier share link, which joins
-- the linked schedule item's live start_date (supplier_share_orders). Make the operator
-- RPC do the same: prefer the linked schedule item's live date, fall back to the stored
-- deliveryDate for orders not tied to a schedule item.

CREATE OR REPLACE FUNCTION public.drywall_supplier_orders()
RETURNS TABLE (
  project_id uuid,
  project_name text,
  order_id text,
  order_number text,
  supplier_id text,
  supplier text,
  delivery_date text,
  status text,
  item_count int,
  updated_at text
)
LANGUAGE sql
STABLE
SECURITY INVOKER
SET search_path = public
AS $$
  SELECT
    p.id,
    p.name,
    NULLIF(trim(o.elem->>'id'), ''),
    NULLIF(trim(o.elem->>'orderNumber'), ''),
    NULLIF(trim(o.elem->>'supplierId'), ''),
    NULLIF(trim(o.elem->>'supplier'), ''),
    COALESCE(si.start_date::text, NULLIF(trim(o.elem->>'deliveryDate'), '')),
    COALESCE(NULLIF(trim(o.elem->>'status'), ''), 'draft'),
    COALESCE(
      jsonb_array_length(
        CASE WHEN jsonb_typeof(o.elem->'items') = 'array' THEN o.elem->'items' ELSE '[]'::jsonb END
      ),
      0
    ),
    NULLIF(trim(o.elem->>'updatedAt'), '')
  FROM public.projects p
  CROSS JOIN LATERAL jsonb_array_elements(
    CASE WHEN jsonb_typeof(p.metadata->'legacy'->'orders') = 'array'
         THEN p.metadata->'legacy'->'orders'
         ELSE '[]'::jsonb END
  ) AS o(elem)
  LEFT JOIN public.schedule_items si
    ON si.id::text = NULLIF(trim(o.elem->>'scheduleItemId'), '')
   AND si.organization_id = p.organization_id;
$$;

REVOKE ALL ON FUNCTION public.drywall_supplier_orders() FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.drywall_supplier_orders() TO authenticated;
