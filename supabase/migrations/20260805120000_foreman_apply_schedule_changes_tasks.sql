-- Extend foreman_apply_schedule_changes to also persist the item's task checklist.
-- tasks is applied only when the batch item carries a 'tasks' array (present →
-- authoritative; absent → left unchanged), matching the notes handling. Cascaded
-- sibling rows echo their existing tasks in the batch, so they no-op.
