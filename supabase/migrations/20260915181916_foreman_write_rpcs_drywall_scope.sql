-- Schedule unification Step 3a part 2: scope drywall-foreman writes to drywall rows.
-- A GC item id must no-op (apply) or raise "schedule item not found" (delete),
-- not mutate GC work on a crossover job.
...
COMMENT ON FUNCTION public.foreman_apply_schedule_changes(uuid, jsonb) IS
  'Drywall foreman update path. The division = drywall filter is deliberate — a GC row on a shared job must not be editable through this RPC.';
...
GRANT EXECUTE ON FUNCTION public.foreman_apply_schedule_changes(uuid, jsonb) TO service_role;
...
BEGIN
  ...
  WHERE id = p_item_id
    ...
    AND division = 'drywall';

  -- Predecessor-strip is integrity cleanup, not a write to GC work. Leave it
  -- unfiltered: if a cross-division predecessor ever exists, a GC row must not
  -- keep a dangling ref to an id that no longer exists.
  UPDATE public.schedule_items si
...
COMMENT ON FUNCTION public.foreman_delete_schedule_item(uuid) IS
  'Drywall foreman delete path. Lookup and DELETE are scoped to division = drywall so a GC item id raises schedule item not found. The sibling predecessor-strip stays unfiltered — integrity cleanup, not a write to GC work.';
...
GRANT EXECUTE ON FUNCTION public.foreman_delete_schedule_item(uuid) TO service_role;
