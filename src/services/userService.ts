  ...
  for (const row of data ?? []) {
    ...
    if (employeeId) {
      links.push({
        ...
        personType: 'employee',
        personId: employeeId,
        personName: employeeNameById.get(employeeId) ?? `Missing team member (${employeeId})`,
        email: row.email ?? '',
      ...
      links.push({
        ...
        personName: contractorNameById.get(contractorId) ?? `Missing team member (${contractorId})`,
        email: row.email ?? '',
        isActive: row.is_active !== false,
        updatedAt: row.updated_at ?? new Date().toISOString(),
      })
