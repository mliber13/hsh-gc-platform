...
const WORKSPACE_ACCESS: Record<RbacRole, Record<Workspace, WorkspaceAccessLevel>> = {
  ...
  office_gc: {
    ...
    tenants: 'write',
    meeting: 'write',
    // GC cross-project Schedule workspace is hidden for now (owner-only) — users were
    // clicking it instead of the Drywall workspace's own schedule. Re-enable when the
    // GC schedule is ready for general use.
    schedule: 'none',
    hr: 'read',
    drywall: 'read',
  },
